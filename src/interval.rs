...
impl<T> Interval<T> {
        ...
        T: Clone + PartialOrd + core::ops::Sub<Output: Zero>,
    {
        ...
        let (a, b) = sided_bounds(self.as_ref());

        let Some(start) = a.bound_val().copied() else {
            return Size::Infinite;
        };
        let Some(end) = b.bound_val().copied() else {
            return Size::Infinite;
        };

        match end.partial_cmp(start) {
            None | Some(Ordering::Less) => Size::Empty,
            Some(Ordering::Equal) => Size::SinglePoint,
            Some(Ordering::Greater) => Size::Finite(end.clone() - start.clone()),
        }
    }
