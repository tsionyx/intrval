...
#[cfg(feature = "arbitrary")]
pub use self::arbitrary::Bounded;
...
pub enum Interval<T> {
    ...
    /// The set of values equal to the given one.
    ///
...
impl<T> Interval<T> {
    ...
    #[must_use = "this method consume the Interval and return a new one"]
    /// Transform the given [`Interval`] into an _open_ one
    ...
    /// Whether the given [`Interval`] contains no valid values,
    ...
    ///
    /// assert_eq!(interval!(_: u8).len().unwrap(), 0);
    /// assert!(interval!(< 5).len().is_none());
    /// assert!(interval!(<= 3.2).len().is_none());
    /// assert!(interval!(> -10).len().is_none());
    /// assert!(interval!(>= 0u32).len().is_none());
    /// assert_eq!(interval!(= 42).len().unwrap(), 0);
    /// assert_eq!(interval!(== 42).len().unwrap(), 0);
    ///
    /// assert_eq!(interval!((1, 10)).len().unwrap(), 9);
    /// assert_eq!(interval!((0, =100)).len().unwrap(), 100);
    /// assert_eq!(interval!((=0, 100)).len().unwrap(), 100);
    /// assert_eq!(interval!((=6, 5)).len().unwrap(), 0);
    /// assert_eq!(interval!([0.0, 1.0]).len().unwrap(), 1.0);
    /// assert_eq!(interval!([5.0, 1.0]).len().unwrap(), 0.0);
    /// assert_eq!(interval!((=0.0, =1.0)).len().unwrap(), 1.0);
    /// assert!(interval!(..: i32).len().is_none());
    /// ```
    pub fn len(&self) -> Option<<T as core::ops::Sub>::Output>
    where
    ...
    /// Whether the given [`Interval`] contains every value.
