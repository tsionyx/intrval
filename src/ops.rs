    ...
    Interval<Z>: Bounded<Z>,
{
    ...
    fn add(self, rhs: Interval<U>) -> Self::Output {
        ...
        let (lhs_start, lhs_end) = if self.is_empty() {
            (
                SidedBound::new(Bound::Included(T::zero())),
                SidedBound::new(Bound::Included(T::zero())),
            )
        } else {
            sided_bounds(self)
        ...
        let (rhs_start, rhs_end) = if rhs.is_empty() {
            (
                SidedBound::new(Bound::Included(U::zero())),
                SidedBound::new(Bound::Included(U::zero())),
            )
        } else {
            sided_bounds(rhs)
        ...
        Interval::from((lhs_start + rhs_start, lhs_end + rhs_end))
    }
}
    ...
    Interval<Z>: Bounded<Z>,
{
    ...
    fn sub(self, rhs: Interval<U>) -> Self::Output {
        ...
        Interval::from((lhs_start - rhs_end, lhs_end - rhs_start))
    }
}
